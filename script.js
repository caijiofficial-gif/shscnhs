...
function resetStep2(level) {
  if (level <= 1) {
    ...
    hide("electives-academic");
    hide("electives-techpro");
    acSelection.clear();
    tpSelection.clear();
  }
  if (level <= 2) {
...
function buildElectiveChips(containerId, items, type, selSet, countId) {
  ...
  items.forEach((label, i) => {
    ...
    chip.className   = `elective-chip ${type}`;
    chip.dataset.idx = i;
    chip.innerHTML   = `<div class="ec-check ${type}"></div><span>${label}</span>`;

    chip.addEventListener("click", () => {
      const check = chip.querySelector(".ec-check");
      if (selSet.has(i)) {
        selSet.delete(i);
      ...
      if (cnt) {
        cnt.textContent = selSet.size + " cluster(s) selected";
        cnt.className   = "selected-count" + (selSet.size > 0 ? " has-selection" : "");
      }
    });
...
window.s2OnTrackChange = function (track) {
  ...
  if (track === "Academic Strand") {
    buildElectiveChips("academic-chips", ACADEMIC_ELECTIVES, "academic", acSelection, "academic-count");
    show("electives-academic");
  } else if (track === "Technical Professional") {
    buildElectiveChips("techpro-chips", TECHPRO_ELECTIVES, "techpro", tpSelection, "techpro-count");
    show("electives-techpro");
  }
